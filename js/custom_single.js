var query_find = function(movieId){
	var status = $('#status');
	var preloader = $('#preloader');
	var body = $('body');
	var movieIdLst = new Array();
	movieIdLst[0] = Number(movieId);
	...
	var movies = Bmob.Object.extend("movies");
	var query = new Bmob.Query(movies);
	query.containedIn("movieId", movieIdLst);
	query.find().then(res => {
		$("#imgId").attr('src',res[0].get("poster"));
		$("#titleId").text(res[0].get("name"));
		$("#title2Id").text(res[0].get("name"));
		$("#ratingId").text(res[0].get("ratingValue"));
		$("#directorId").text(res[0].get("director"));
		$("#starsId").text(res[0].get("actors").replace(/\|/g,','));
		$("#genresId").text(res[0].get("genres").replace(/\|/g,','));
		$("#yearId").text(res[0].get("year"));
		$("#countryId").text(res[0].get("country"));
		$("#languageId").text(res[0].get("language"));
		var MoviesDB = Bmob.Object.extend("movies");
		var query_2 = new Bmob.Query(MoviesDB);
		query_2.containedIn("movieId", res[0].get("recommend"));
		query_2.find().then(res => {
			$("#recCountId").text(res.length);
			var data = "";
			for (var i = 0; i < res.length; i++) {
				data = data + "<div class='movie-item-style-2'><img src='" 
				+ res[i].get("poster") +"' alt=''><div class='mv-item-infor'><h6><a href='moviesingle.html?movieid="
				+ res[i].get('movieId')+"'>"
				+ res[i].get("name") + "</a></h6><p class='rate'><i class='ion-android-star'></i><span>"
				+ res[i].get("ratingValue") + "</span> /10</p><p class='describe'> describe...</p><p>Director: <a href='#'>"
				+ res[i].get("director") + "</a></p><p>Stars: <a href='#'>"
				+ res[i].get("actors").replace(/\|/g,',') + "</a></p></div></div>"
			}
			
			document.getElementById("relatedmoviesDiv").innerHTML = data;			
		});	
